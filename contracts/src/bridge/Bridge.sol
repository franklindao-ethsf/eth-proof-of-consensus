...
contract Deposit is Bridge {
    ...
    constructor(
        ITrustlessAMB _homeAmb,
        address _foreignWithdraw,
        uint16 _chainId
    ) {
        homeAmb = _homeAmb;
    ...
    function deposit(
        ...
        IERC20(tokenAddress).transferFrom(msg.sender, address(this), amount); // lock
...
contract DepositMock is Deposit {
    constructor(
        ...
        address _foreignWithdraw,
        uint16 _chainId
    ) Deposit(_homeAmb, _foreignWithdraw, _chainId) {}
...
contract Withdraw is Bridge {
    ...
    {
        require(
            msg.sender == foreignAmb,
            "Only foreign amb can call this function"
        );
        require(
            srcAddress == homeDeposit,
            "Only home deposit can trigger a message call to this contract."
        );
        (address recipient, uint256 amount, address tokenAddress) = abi.decode(
            callData,
